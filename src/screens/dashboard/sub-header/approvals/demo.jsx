    ...
    <View style={{ marginBottom: 16 }}>
      ...
      <TouchableOpacity
        ...
        <Text style={{ fontSize: 14, color: selectedOption ? '#1f2937' : '#9ca3af' }}>
          {selectedOption ? selectedOption.dropdownValue : placeholder}
        </Text>
        <Icon 
...
// Form Modal Component (updated with member suggestions and status dropdown)
const FormModal = ({ visible, onClose, onSubmit, selectedItem, members, statusOptions }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Prepare snag data - use IDs for reportedBy, responsibleParty, and status
      const snagData = {
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={{ marginBottom: 16 }}>
              ...
              <TextInput
                ...
                onChangeText={(text) => handleInputChange('drawingReference', text)}
              />
            </View>
...
const SnaggingReportScreen = () => {
  ...
  // Fetch initial data, members, and status options
  useEffect(() => {
    fetchInitialData();
    fetchMembers();
    fetchStatusOptions();
