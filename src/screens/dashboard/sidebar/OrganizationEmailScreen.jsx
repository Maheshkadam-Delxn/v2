...
export default function OrganizationEmailScreen({ toggleMainSidebar, isMainSidebarOpen }) {
  const [selectedFolder, setSelectedFolder] = useState('All');
  const [showFolderDropdown, setShowFolderDropdown] = useState(true);
  const [composeModalVisible, setComposeModalVisible] = useState(false);
  const [sidebarOpen, setSidebarOpen] = useState(false); // Default to closed
  ...
  const toggleSidebar = () => {
    if (!sidebarOpen && isMainSidebarOpen) {
      toggleMainSidebar(); // Close MainLayout sidebar if open
    }
    setSidebarOpen(!sidebarOpen);
  };
  ...
  const handleSidebarItemPress = (folderName) => {
    setSelectedFolder(folderName);
    setSidebarOpen(false); // Close local sidebar
    if (isMainSidebarOpen) {
      toggleMainSidebar(); // Close MainLayout sidebar
    }
  };
  ...
  return (
    <MainLayout title="My Email">
      <View className="flex-1 flex-row bg-gray-50">
        {/* Sidebar with Toggle */}
        <View
          className={`${sidebarOpen ? 'w-72' : 'w-16'} bg-white border-r border-gray-200 ${sidebarOpen ? 'p-5' : 'p-2'} shadow-sm transition-all duration-300 relative z-20`}
        >
          <TouchableOpacity
            className="absolute -right-3 bg-blue-600 rounded-full p-1 z-10 shadow-md"
            onPress={toggleSidebar}
            ...
            <>
              <TouchableOpacity
                ...
                onPress={() => {
                  setComposeModalVisible(true);
                  setSidebarOpen(false);
                  if (isMainSidebarOpen) toggleMainSidebar();
                }}
              >
                <Ionicons name="add" size={22} color="white" />
                <Text className="text-white font-semibold ml-2 text-base">Compose Mail</Text>
