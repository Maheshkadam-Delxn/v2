...
export default function DashboardScreen() {
  ...
  const handleFilterSelect = (filter) => {
    // Check if the filter is for Inventory (use label or autoId based on API data)
    if (filter.label === 'Inventory' || filter.autoId === 'INVENTORY_ID') { // Replace 'INVENTORY_ID' with actual autoId if known
      navigation.navigate('Inventory');
      setSelectedFilter(null);
      setSelectedSubItem(null);
    } else if (selectedFilter?.autoId === filter.autoId) {
  ...
  const FilterSection = () => (
    ...
    >
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingHorizontal: 24 }}
      >
        {dropdownoptions && Array.isArray(dropdownoptions) && dropdownoptions.length > 0 ? (
          dropdownoptions.map((menu, index) => {
            const isSelected = selectedFilter?.autoId === menu.autoId;
            const hasItems = menu.submenu && Array.isArray(menu.submenu) && menu.submenu.length > 0;

            return (
              <TouchableOpacity
                key={menu.autoId}
                onPress={() => handleFilterSelect(menu)}
                style={{
                  marginRight: 12,
                  paddingHorizontal: 20,
                  paddingVertical: 12,
          ...
          <Text style={{ color: colors.text, fontSize: 14 }}>
            {isLoading ? 'Loading filters...' : 'Error: Filter options unavailable'}
          </Text>
        )}
      </ScrollView>

      {/* Submenu section */}
      {selectedFilter && selectedFilter.submenu && Array.isArray(selectedFilter.submenu) && selectedFilter.submenu.length > 0 && (
        <Animated.View
          style={{
            marginTop: 16,
            marginHorizontal: 24,
